...
static bool rkvenc_get_ccu(struct device *dev, struct rkvenc_ccu **ret_ccu) {
...
static int rkvenc2_alloc_rcbbuf(struct platform_device *pdev, struct rkvenc_dev *enc)
{
	...
	dma_alloc_coherent(dev, sram_used, &iova, GFP_KERNEL);
	// if (ret) {
...
static int rkvenc_core_probe(struct platform_device *pdev)
{
	...
	if (pdev->dev.of_node) {
		...
		if (match)
			mpp_var = (struct mpp_dev_var *)match->data;

		core_id = of_alias_get_id(np, "rkvenc");
	}

	if (!rkvenc_get_ccu(dev, NULL)) {
		dev_err(dev, "couldn't get ccu, deferring\n");
		return -EPROBE_DEFER;
	}
